...
async def run_test():
    ...
    try:
        ...
        # -> Fill the username and password fields and submit the login form by clicking the 'Sign In' button.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Open the Setup menu to find the Device Profiles link (expand Setup).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/aside/div/nav/div[2]/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'Device Profiles' link under the Setup menu to open the Device Profiles page.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/aside/div/nav/div[2]/div/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'New Profile' button to open the create device profile form.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div[2]/main/div/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill 'Profile Name' and 'Manufacturer' fields, then click the Create Profile button to create 'Test Profile A'.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[3]/div/div[2]/div/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Test Profile A')
        
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[3]/div/div[2]/div/div[2]/input').nth(0)
