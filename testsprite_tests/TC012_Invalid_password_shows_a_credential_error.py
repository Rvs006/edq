...
async def run_test():
    ...
    try:
        ...
        # -> Fill the username field with 'admin', fill the password field with 'invalid-password', then click the 'Sign In' submit button.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the 'Sign In' submit button to retry login with the incorrect password, then wait for the response and check for a visible credential validation error message.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/main/div/div[3]/div/div[2]/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
