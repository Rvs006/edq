...
import { ACTIVE_UNIVERSAL_TESTS, UNIVERSAL_TESTS } from '@/lib/universal-tests'
...
function getProfileVisual(category?: string, autoGenerated?: boolean): { icon: React.ElementType; color: string } {
  if (autoGenerated) return { icon: Sparkles, color: 'indigo' }
  switch (category) {
    case 'camera':
      return { icon: Camera, color: 'blue' }
    case 'controller':
    case 'access_panel':
    ...
    case 'hvac':
    case 'iot_sensor':
      return { icon: Radio, color: 'green' }
    default:
      return { icon: Box, color: 'zinc' }
  }
}
...
export default function DeviceProfilesPage() {
  ...
  const canManageProfiles = user?.role === 'admin'

  const { data: apiProfiles = [], isLoading, isError } = useQuery({
    queryKey: ['device-profiles'],
    queryFn: () => profilesApi.list({ limit: 200 }).then((r) => r.data),
  })

  const savedProfiles: DeviceProfile[] = (apiProfiles as ApiProfile[])
    .map((profile) => ({
      ...getProfileVisual(profile.category, profile.auto_generated),
      id: profile.id,
      name: profile.name,
      description: profile.description || `Fingerprint profile for ${profile.manufacturer || profile.category || 'unknown devices'}`,
      testIds: profile.fingerprint_rules?.skip_test_ids
        ? ACTIVE_UNIVERSAL_TESTS.map((test) => test.id).filter((testId) => !profile.fingerprint_rules?.skip_test_ids?.includes(testId))
        : ACTIVE_UNIVERSAL_TESTS.map((test) => test.id),
      autoGenerated: profile.auto_generated,
    }))

  const allProfiles = [...BUILTIN_PROFILES, ...savedProfiles]
    ...
    <div className="page-container">
      ...
      {isError && (
        <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-800 dark:bg-red-950/30 dark:text-red-300">
          Saved device profiles could not be loaded. Built-in reference profiles are shown below, but your database-backed profiles may be unavailable.
        </div>
      )}
      ...
      {selected && (
        <div className="card p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            {selectedApiProfile && canManageProfiles && (
