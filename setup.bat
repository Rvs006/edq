...
for /f "usebackq delims=" %%A in (`powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$defaults = @(@{Key='ENVIRONMENT';Value='docker'}, @{Key='DB_DRIVER';Value='postgresql+asyncpg'}, @{Key='DB_HOST';Value='127.0.0.1'}, @{Key='DB_PORT';Value='55432'}, @{Key='DB_NAME';Value='edq'}, @{Key='DB_USER';Value='edq'}, @{Key='DB_PASSWORD';Value=$postgresPassword}, @{Key='DB_CONNECT_TIMEOUT_SECONDS';Value='15'}, @{Key='EDQ_BACKEND_BIND_HOST';Value='127.0.0.1'}, @{Key='EDQ_BACKEND_PORT';Value='8000'}, @{Key='EDQ_TOOLS_BIND_HOST';Value='127.0.0.1'}, @{Key='EDQ_TOOLS_PORT';Value='8001'}, @{Key='EDQ_POSTGRES_BIND_HOST';Value='127.0.0.1'}, @{Key='EDQ_POSTGRES_PORT';Value='55432'}, @{Key='EDQ_PUBLIC_URL';Value=''}, @{Key='VITE_API_URL';Value='/api'}, @{Key='VITE_CLIENT_ERROR_ENDPOINT';Value='/api/client-errors'}, @{Key='VITE_SENTRY_ENABLED';Value='false'}, @{Key='VITE_SENTRY_TRACES_SAMPLE_RATE';Value='0.0'}, @{Key='VITE_SOURCEMAP';Value='false'}, @{Key='LOG_JSON';Value='false'}); " ^
...
for /L %%I in (1,1,30) do (
  docker compose exec -T backend wget -qO - http://localhost:8000/api/v1/health >nul 2>nul
  if not errorlevel 1 (
    set "READY=1"
    goto :ready
  )
