...
- `docker-compose.prod.yml` — built-in production HTTPS path using nginx + certbot (`docker compose -f docker-compose.yml -f docker-compose.prod.yml up`).
- `docker-compose.tls.yml` — optional Caddy TLS overlay (`docker compose -f docker-compose.yml -f docker-compose.tls.yml up`). Uses `Caddyfile` at repo root.
